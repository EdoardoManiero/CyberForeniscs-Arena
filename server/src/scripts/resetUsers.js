...
async function deleteAllUsers() {
  ...
  // Order: badge_points_awarded -> user_badges -> task_completions -> user_devices -> user_vfs_state -> user_stats -> users
...
async function deleteUserByEmail(email) {
  ...
  console.log(`  Found ${devicesCount.count} device(s), ${vfsCount.count} VFS state(s), ${badgePointsCount.count} badge point award(s), ${badgesCount.count} badge(s), ${completionsCount.count} completion(s)`);
  ...
  console.log(`  Deleted ${devicesResult.changes} device(s), ${vfsResult.changes} VFS state(s), ${badgePointsResult.changes} badge point award(s), ${badgesResult.changes} badge(s), ${completionsResult.changes} completion(s), ${statsResult.changes} stat(s)`);
  ...
  if (remainingDevices.count > 0 || remainingVFS.count > 0 || remainingBadgePoints.count > 0) {
    console.warn(`        Warning: Some data may still exist (${remainingDevices.count} devices, ${remainingVFS.count} VFS states, ${remainingBadgePoints.count} badge points)`);
  }
}
